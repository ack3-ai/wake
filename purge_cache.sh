...
SITEMAP_URL="https://ack3.ai/wake/docs/latest/sitemap.xml"
PURGE_URLS=$(curl -s $SITEMAP_URL | grep -Eo '<loc>[^<]*' | cut -d '>' -f 2)

PURGE_URLS+=" https://ack3.ai/wake/docs/versions.json"
PURGE_URLS+=" https://ack3.ai/wake/docs/latest/search/search_index.json"
